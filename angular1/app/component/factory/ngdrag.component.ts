            ...
            <div class="controller" >
                ...
                <ul class="reception" ng-drop=" callback : '$ctrl.add($ctrl.pItem1,$transport)' " >
                    <li ng-repeat="item in $ctrl.pItem1">
                        <div  ng-drag=" namespace : 'groupe1' ,
                                        transport : 'item' ,
                                        callback : '$ctrl.remove($ctrl.pItem1,$index)' ">
                            {{item.name}}
                        </div>
                    </li>
                </ul>
                ...
                <div class="scope"> {{$ctrl.pItem2}}</div>
                <ul class="reception"
                    ng-drop=" namespace : 'groupe1' ,
                              constraint: '$ctrl.isValid($transport)' ,
                              callback : '$ctrl.add($ctrl.pItem2,$transport)' "  >
                    <li ng-repeat="item in $ctrl.pItem2"   >
                        <div ng-drag=" transport : 'item' ,
                                       callback : '$ctrl.remove($ctrl.pItem2,$index)'  " >
                         {{item.name}}
                        </div>
                    </li>
             </ul>
