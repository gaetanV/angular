...
angular.module('gaetan').directive('childrenRepeat', [
  function () {
    return {
      restrict: 'A',
      transclude: true,
    ...
    function compile($element, $attrs, transclude) {
      ...
      try {
        if (!$attrs.childrenRepeat)
          throw 'you should specified children-repeat attribute';
        var match = $attrs.childrenRepeat.match(
          /^\s*(.+)\s+in\s+(.*?)\s*track\s+by\s+(.+)\s*?$/
        );
        indexName = match[1];
        collectionName = match[2];
        scopeChildName = match[3];
        if (!indexName || !collectionName || !scopeChildName)
            ...
            function buildListNode(collection, parent, depth) {
              ...
              if (depth !== 0) {
                var container = angular.element(
                  document.createElement(nodeParentName)
                );
              ...
              for (var i = 0; i < collection.length; i++) {
                ...
                transclude(childScope, function (clone) {
                   ...
                   */
                  var haveChild =
                    collection[i][scopeChildName] &&
                    collection[i][scopeChildName].length > 0
                      ? true
                      : false;
