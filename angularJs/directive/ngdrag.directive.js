 ...
 *
 */
angular.module('gaetan').directive('ngDrag', [
  '$compile',
  '$parse',
  function ($compile, $parse) {
    ...
    var drag = function (e, namespace, constraint, callback) {
      ...
      vm.checkTimer = setInterval(function check() {
        if (vm.haveMove) vm.haveMove = false;
      }, 50);
    ...
    drag.prototype.end = function () {
      ...
      if (this.valid) {
        this.dropTo(this.ngdropList[this.overDrop]);
      }
      this.removeDropList();
    };
    ...
    drag.prototype.checkPosition = function (e) {
      ...
      for (var i = 0; i < drop.length; i++) {
        if (
          x > drop[i].offsetLeft &&
          x < drop[i].offsetWidth + drop[i].offsetLeft &&
          y > drop[i].offsetTop &&
          y < drop[i].offsetTop + drop[i].offsetHeight
        ) {
          inside = true;
          if (this.overDrop != i) {
            /**
    ...
    drag.prototype.removeDropList = function () {
      angular
        .element(this.ngdropList[this.overDrop])
        .removeClass('ng-drop-error');
    ...
    function link($scope, element, attrs, module, $transclude) {
      ...
      if (!optionDrag.namespace) optionDrag.namespace = '';
      ...
      var isValid = function (target, callback) {
        ...
        if (!optionDrop.constraint) callback(true);
        else
