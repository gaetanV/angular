 ...
 *          in {string}
 *          track by {string}
 *  @exemple : [  children-option = "id as name in sample track by children" ]
 *
 *TO DO OPTION SELECT AND BUILD
 *
 */

angular.module('gaetan').directive('childrenOption', [
  '$compile',
  function ($compile) {
    return {
      restrict: 'A',
    ...
    function link($scope, $element, $attrs, $controller) {
      ...
      try {
        ...
        var match = $attrs.childrenOption.match(
          /^\s*(.+)\s+as\s*(.+)\s+in\s+(.*?)\s*track\s+by\s+(.+)\s*?$/
        );
        indexName = match[1];
        indexText = match[2];
      ...
      var initDom = function () {
        ...
        ulDom.setAttribute(
          'children-repeat',
          'finderField in finderFields track by ' + scopeChildName + ''
        );
        liDom = document.createElement('li');
        ...
        var hide = function (e) {
          ...
          if (e.target == inputDom) return false;
